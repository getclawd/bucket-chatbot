...
This keeps what made the original interesting — it genuinely learns from the
message content it receives — and rebuilds the machinery so it holds up: SQLite
instead of flat files, a real n-gram model, semantic recall via content-only
embeddings, and an optional LLM layer that smooths delivery without ever
inventing content. Account identity and message attribution are deliberately
...
user typed. It picks between several strategies each turn:
...
| `pair` | finds a past message like yours and returns the adjacent reply |
...
The top four **compose** — they collide two things it knows rather than
...
The runtime memory is content-only. The SQLite schema does not store account
usernames, message authors, channel/chat IDs, inventory givers, or alias maps,
and the bot has no attribution strategy or identity pages. The names of people
typed into a message are still message content; set `BUCKET_BLOCKED_NAMES` and
run `python scrub_legacy.py --apply` if a deployment must suppress particular
names from generation and display.

Adapters may transiently inspect platform IDs to ignore bot messages or enforce
admin-only commands. Conversation keys used for short-term context and pacing
are in memory only; they never reach SQLite or a network API. Embeddings remain
enabled, but they represent message/fact content for semantic recall, not who
provided it.

Before publishing a folder archive, remove `.env`, `bucket.sqlite3*`, and other
runtime artifacts. They are ignored by Git, but ignored files can still be
included in a ZIP. A fresh clone starts with the privacy-first schema and no
private corpus.
...
on later retrofits everything Bucket already knows. They are content-recall
vectors only: there is no attribution vector and no identity metadata attached
to them.
...
It learns from message content by default, with no topic filter. Levers if you
change your mind, none of which require touching code:
...
Before sharing a folder archive, confirm that it contains no `.env`,
`.env.*.bak`, `bucket.sqlite3*`, `__pycache__`, or local settings. `.gitignore`
keeps these out of commits, but Git does not stop them from entering a ZIP.
The committed `.env.example` contains placeholders only; each deployment should
create its own `.env` locally.
...
bucket/
  ...
  brain.py    response strategies + markov generation
...
python privacy_test.py        # legacy identity data is migrated and discarded
...
factoid recall works, items get picked up, every response strategy fires, an
