...
def main() -> int:
    ...
    # The privacy-first schema keeps no provenance, so this maintenance pass
    # intentionally re-mines every stored content line, including old replies.
    rows = db.conn.execute("SELECT text FROM utterances ORDER BY id").fetchall()
    print(f"reading {len(rows)} stored lines\n")
    ...
    for row in rows:
        learner._learn_factoid(row["text"])
