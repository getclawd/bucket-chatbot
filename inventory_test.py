...
def main() -> int:
    ...
    bot.learner.ingest("someone gives bucket a rusty hammer")
    bot.learner.ingest("bucket picks up the knife")
    ...
    bot.learner.ingest("bucket holds the knife")
    ...
    bot.learner.ingest("bucket gives someone the rusty hammer")
    ...
    before = bot.db.items()
    bot.learner.ingest("bucket drops a nonexistent sword")
    ...
    bot.learner.ingest(
        "bucket gives someone the knife",
        reinforce=False,
    )
    ...
    capacity_db.add_item("one", 2)
    capacity_db.add_item("two", 2)
    dropped = capacity_db.add_item("three", 2)
    ...
    for index, phrase in enumerate((
        ...
        phrase_bot.db.add_item("knife", 12)
