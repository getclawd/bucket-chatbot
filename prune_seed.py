...
The seed exists so a fresh bot isn't mute. Once real conversation exists, the
seed stops being scaffolding and becomes noise — worse, generic filler
...
def main() -> int:
    ...
    seed_norms = {normalize(line) for line in SEED_LINES}
    rows = [
        row for row in conn.execute("SELECT id, text, count FROM utterances")
        if normalize(row["text"]) in seed_norms
    ]

    # Provenance is intentionally not stored. Recreate the small seed corpus in
    # memory so its derived facts can be identified from content alone.
    scratch = BucketDB(":memory:")
    seed_learner = Learner(scratch)
    for line in SEED_LINES:
        seed_learner.ingest(line)
    seed_facts = {
        (row["subject"], row["verb"], row["object"])
        for row in scratch.conn.execute("SELECT subject, verb, object FROM factoids")
    }
    scratch.close()
    fact_rows = [
        row for row in conn.execute("SELECT id, subject, verb, object FROM factoids")
        if (row["subject"], row["verb"], row["object"]) in seed_facts
    ]
    ...
    if not rows:
        ...
        print(f"seed-derived facts to remove: {len(fact_rows)}")
        if not apply:
            print("\nthis was a dry run. re-run with --apply to do it.")
            return 0
        for row in fact_rows:
            conn.execute("DELETE FROM factoids WHERE id = ?", (row["id"],))
        conn.execute(
    ...
    for row in conn.execute(
        "SELECT count, text FROM utterances ORDER BY count DESC LIMIT 8"
    ):
        print(f"  x{row['count']:<3} {row['text'][:52]}")
