...
def main() -> int:
    ...
    bot.learner.ingest(LINE, author=config.NAME, chat="tg:1", reinforce=False)
    ...
    for _ in range(4):
        bot.learner.ingest(LINE, author="Elle", chat="tg:1")
    ...
    uid = bot.learner.ingest(novel, author=config.NAME, chat="tg:1", reinforce=False)
    ...
    first = probe.learner.ingest("humans said this first", author="Elle", chat="c")
    probe.learner.ingest(
        "an entirely novel sentence bucket invented itself",
        author=config.NAME, chat="c", previous_id=first, reinforce=False,
    )
    ...
    probe2.learner.ingest("humans said this first", author="Elle", chat="c")
    ...
    for index in range(60):
        bot2.handle(f"message {index} about assorted things", author="Elle",
                    chat="tg:2", is_private=True)

    top = bot2.db.conn.execute(
        "SELECT count, author, text FROM utterances ORDER BY count DESC LIMIT 5"
    ).fetchall()
    print("  most repeated after the run:")
    for row in top:
        print(f"    x{row['count']:<3} [{row['author'] or '?':<8}] {row['text'][:48]}")

    self_authored = [r for r in top if r["author"] == config.NAME and r["count"] > 1]
    if self_authored:
        failures.append(
            f"bucket's own line reached count {self_authored[0]['count']} unaided"
        )
