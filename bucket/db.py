...
Five things live in here:
  utterances   every line anyone has said, with a repeat counter
  pairs        "when someone said A, someone else replied B" (Cleverbot-style)
  chain        n-gram transition counts (order 2 with order 1 backoff)
  factoids     "X is Y" triples
  inventory    the things people have handed it
"""
...
class BucketDB:
    ...
    # utterances
    # ------------------------------------------------------------------
    def add_utterance(self, text: str, author: str = "", chat: str = "",
                      reinforce: bool = True) -> int | None:
        ...
        cur = self.conn.execute(
            "INSERT INTO utterances (text, norm, author, chat, ts) VALUES (?, ?, ?, ?, ?)",
            (text, norm, author, chat, time.time()),
        )
    ...
    # factoids
    # ------------------------------------------------------------------
    def add_factoid(self, subject: str, verb: str, obj: str, author: str = "") -> None:
        # File everything under the canonical name so "@ana" and "anabird"
        # don't become separate people.
        subject = self.resolve(subject)
        self.conn.execute(
            "INSERT INTO factoids (subject, verb, object, author, ts) VALUES (?, ?, ?, ?, ?) "
            "ON CONFLICT(subject, verb, object) DO UPDATE SET count = count + 1",
            (subject.lower(), verb.lower(), obj, author, time.time()),
        )
        self.conn.commit()
    ...
    # ------------------------------------------------------------------
    # aliases — so ana / Ana / @ana / anabird are one person
    # ------------------------------------------------------------------
    def add_alias(self, alias: str, canonical: str) -> bool:
        alias, canonical = alias.lower().strip(), canonical.lower().strip()
        if not alias or not canonical or alias == canonical:
            return False
        # Don't let A->B and B->A form a cycle.
        if self.resolve(canonical) == alias:
            return False
        self.conn.execute(
            "INSERT INTO aliases (alias, canonical, ts) VALUES (?, ?, ?) "
            "ON CONFLICT(alias) DO UPDATE SET canonical = excluded.canonical",
            (alias, canonical, time.time()),
        )
        self.conn.commit()
        return True

    def resolve(self, name: str, _depth: int = 0) -> str:
        """Follow an alias to the name facts are actually filed under."""
        name = name.lower().strip().lstrip("@")
        if _depth > 4:  # paranoia against a chain someone managed to loop
            return name
        row = self.conn.execute(
            "SELECT canonical FROM aliases WHERE alias = ?", (name,)
        ).fetchone()
        return self.resolve(row["canonical"], _depth + 1) if row else name

    def display_name(self, subject: str) -> str:
        """The name people actually say, not the one facts are filed under.

        Facts collapse onto whichever name had the most attached to it, which
        is often a handle. In conversation "ana" is what gets used, so saying
        "ben has wet puh" back to Ana reads as a stranger talking.
        """
        canonical = self.resolve(subject)
        names = [canonical, *self.aliases_of(canonical)]
        # Shortest wins: nicknames are shorter than handles almost always.
    ...
    def aliases_of(self, canonical: str) -> list[str]:
        ...
        return [
            r["alias"]
            for r in self.conn.execute(
                "SELECT alias FROM aliases WHERE canonical = ? ORDER BY alias", (canonical,)
            )
        ]
    ...
    def author_lines(self, author: str, limit: int = 5) -> list[sqlite3.Row]:
        return self.conn.execute(
            "SELECT text, count FROM utterances WHERE lower(author) = ? "
            "ORDER BY count DESC, RANDOM() LIMIT ?",
            (author.lower(), limit),
        ).fetchall()

    def author_total(self, author: str) -> int:
        return self.conn.execute(
            "SELECT COUNT(*) n FROM utterances WHERE lower(author) = ?", (author.lower(),)
        ).fetchone()["n"]
    ...
    def known_authors(self) -> list[str]:
        return [
            r["author"]
            for r in self.conn.execute(
                "SELECT DISTINCT author FROM utterances WHERE author IS NOT NULL AND author != ''"
            )
        ]
    ...
    # inventory
    # ------------------------------------------------------------------
    def add_item(self, item: str, giver: str, capacity: int) -> str | None:
        """Take an item. Returns the item dropped to make room, if any."""
        item = item.strip()
        if not item:
            return None
        self.conn.execute(
            "INSERT OR IGNORE INTO inventory (item, giver, ts) VALUES (?, ?, ?)",
            (item, giver, time.time()),
        )
        dropped = None
    ...
    def wipe(self) -> None:
        for table in ("utterances", "pairs", "word_index", "words", "chain",
                      "factoids", "inventory", "vectors", "phrases", "aliases"):
