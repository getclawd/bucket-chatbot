...
from .blocklist import DEFAULT_BLOCKED, NON_PERSON_AUTHORS, Blocklist
...
# Authors that can't be credited out loud, because they aren't people.
UNCREDITABLE = NON_PERSON_AUTHORS | {config.NAME}
...
class Brain:
    ...
    def _plan(self, text: str):
        ...
        weights: list[tuple[float, callable]] = [
            ...
            (2.0, self._from_attribution), # "elle says ana has wet puh"
            # single-source
            (4.0, self._from_pair),
    ...
    def _from_factoid(self, text: str) -> Reply | None:
        ...
        for subject in subjects[:3]:
            ...
            name = self.db.display_name(row["subject"])
            templates = [
    ...
    def _from_tangent(self, text: str) -> Reply | None:
        ...
        opening = f"{self.db.display_name(row['subject'])} {row['verb']} {row['object']}".strip()
    ...
    def _from_collision(self, text: str) -> Reply | None:
        ...
        left = (f"{self.db.display_name(first['subject'])} "
                f"{first['verb']} {first['object']}")
        right = (f"{self.db.display_name(second['subject'])} "
    ...
    def _from_phrase(self, text: str) -> Reply | None:
        ...
        if fact is not None:
            claim = f"{self.db.display_name(fact['subject'])} {fact['verb']} {fact['object']}"
    ...
    def _from_attribution(self, text: str) -> Reply | None:
        """Blame a person for something it was told. Very 2008 Bucket."""
        # Only real people get blamed. `webapp` is the Mini App's author tag and
        # `seed`/`bucket2008` aren't people either — crediting them produced
        # "webapp says ana has wet puh", which reads as a person nobody knows.
        marks = ",".join("?" * len(UNCREDITABLE))
        row = self._topical_factoid(text)
        if not row or (row["author"] or "").lower() in UNCREDITABLE:
            row = self.db.conn.execute(
                f"SELECT * FROM factoids WHERE author IS NOT NULL AND author != '' "
                f"AND LOWER(author) NOT IN ({marks}) ORDER BY RANDOM() LIMIT 1",
                tuple(UNCREDITABLE),
            ).fetchone()
        if not row or not row["author"]:
            return None

        fact = f"{self.db.display_name(row['subject'])} {row['verb']} {row['object']}"
        template = random.choice([
            f"{row['author']} told me {fact}",
            f"{row['author']} says {fact}",
            f"according to {row['author']}, {fact}",
            f"don't blame me, {row['author']} said {fact}",
        ])
        return Reply(template, "attribution")
