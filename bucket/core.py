...
class Bucket:
    def __init__(self, db_path: str | None = None, quiet: bool = False):
        ...
        # What people have said in each chat lately, so a reply to "what about
        # him" has something to attach "him" to.
    ...
    def explain(self, text: str, chat: str = "webapp", learn: bool = False) -> dict:
        """Reply, plus everything about how it got there.

        Powers the Mini App's chat view: which strategy fired, what the engine
        produced before polish, and what recall dug up. Defaults to not learning
    ...
    def learn_only(self, text: str, author: str = "", chat: str = "") -> None:
        ...
        if not text or not config.LEARN:
            return
        with self._lock:
            uid = self.learner.ingest(text, author=author, chat=chat)
            if uid is not None:
                self._remember(uid, text)
        ...
        with self._lock:
            ...
            if config.LEARN:
                own_id = self.learner.ingest(
                    out, author=config.NAME, chat=chat,
                    # Its own voice doesn't count as another vote for the line.
                    # Only people repeating something can make it an obsession.
                    reinforce=False,
                )
                if own_id is not None:
                    self._remember(own_id, out)
                # Deliberately not updating _last_utterance: the pair graph
                # should record what people said after each other, not what
                # Bucket interjected. Leaving it means the next human message
                # chains to the previous human message.
    ...
    def cmd_literal(self, subject: str) -> str:
        ...
        if rows:
            ...
            for row in rows[:20]:
                lines.append(
                    f"  {row['verb']} {row['object']}  "
                    f"(x{row['count']}, from {row['author'] or '?'})"
                )
            if len(rows) > 20:
        ...
        if seen:
            ...
            for row in seen:
                ...
                lines.append(f"  [{author}] {self.redact(row['text'])[:70]}")
        return "\n".join(lines)

    def cmd_alias(self, argument: str) -> str:
        """/alias cleo elle — same person, one set of facts.

        Deliberately a command rather than inferred from "X is Y": that shape is
        overwhelmingly an ordinary fact ("cleo is annoying"), and guessing
        wrong silently renames someone.
        """
        parts = [p for p in normalize(argument).replace(" = ", " ").split() if p]
        if len(parts) < 2:
            listed = self.db.conn.execute(
                "SELECT alias, canonical FROM aliases ORDER BY canonical, alias"
            ).fetchall()
            if not listed:
                return "usage: alias <name> <other name>   (e.g. alias cleo elle)"
            out = ["i know these are the same people:"]
            out += [f"  {r['alias']} = {r['canonical']}" for r in listed]
            out.append("\nusage: alias <name> <other name>")
            return "\n".join(out)

        alias, canonical = parts[0], " ".join(parts[1:])
        with self._lock:
            # Point at whichever name already has more attached to it.
            weight_a = len(self.db.factoids_for(alias)) + self.db.author_total(alias)
            weight_b = len(self.db.factoids_for(canonical)) + self.db.author_total(canonical)
            if weight_a > weight_b:
                alias, canonical = canonical, alias

            if not self.db.add_alias(alias, canonical):
                return f"can't link {alias} to {canonical}"

            # Re-file anything already stored under the old name.
            moved = self.db.conn.execute(
                "SELECT COUNT(*) n FROM factoids WHERE subject = ?", (alias,)
            ).fetchone()["n"]
            self.db.conn.execute(
                "UPDATE OR REPLACE factoids SET subject = ? WHERE subject = ?",
                (canonical, alias),
            )
            self.db.conn.commit()

        note = f" (moved {moved} fact(s))" if moved else ""
        return f"{alias} is {canonical}, got it{note}"
    ...
    def cmd_about(self, name: str) -> str:
        """Everything it has on a person: their facts, and what they say."""
        name = normalize(name).lstrip("@")
        if not name:
            return "about who"

        canonical = self.db.resolve(name)
        facts = self.db.factoids_for(canonical)
        other_names = [a for a in self.db.aliases_of(canonical) if a != name]

        # Messages are filed under whichever name that person chats as, which is
        # often an alias rather than the canonical one — so count them all.
        every_name = {canonical, name, *self.db.aliases_of(canonical)}
        total = sum(self.db.author_total(n) for n in every_name)
        lines_said = []
        for n in every_name:
            lines_said.extend(self.db.author_lines(n, limit=5))
        lines_said.sort(key=lambda r: -r["count"])
