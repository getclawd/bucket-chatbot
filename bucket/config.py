...
class Config:
    ...
    # This is what keeps the LLM a voice instead of an author. Off at your peril.
    ...
    # --- telegram mini app ---
    WEBAPP = _get("BUCKET_WEBAPP", "0") not in ("0", "false", "no")
    WEBAPP_HOST = _get("BUCKET_WEBAPP_HOST", "127.0.0.1")
    WEBAPP_PORT = _get_int("BUCKET_WEBAPP_PORT", 8770)
    # The public https:// address your tunnel exposes. Telegram will not open
    # localhost or a self-signed certificate. Leave empty and let Bucket start
    # its own tunnel — a free quick-tunnel URL changes on every restart, so
    # pinning one here goes stale the moment cloudflared is restarted.
    WEBAPP_URL = _get("BUCKET_WEBAPP_URL", "")
    # Start cloudflared automatically and use whatever URL it reports.
    TUNNEL = _get("BUCKET_TUNNEL", "1") not in ("0", "false", "no")
    CLOUDFLARED = _get("BUCKET_CLOUDFLARED", "")
    # The corpus is every message from your group, so it is closed by default.
    WEBAPP_ALLOW_ALL = _get("BUCKET_WEBAPP_ALLOW_ALL", "0") not in ("0", "false", "no")
    # Skip Telegram signature checks so you can open it in a normal browser.
    # Never enable this on a publicly reachable port.
    WEBAPP_DEV = _get("BUCKET_WEBAPP_DEV", "0") not in ("0", "false", "no")
