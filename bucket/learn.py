...
# from the legacy-transcript list plus BUCKET_BLOCKED_NAMES; see blocklist.py.
...
class Learner:
    ...
    def ingest(self, text: str, previous_id: int | None = None,
        ...
        uid = self.db.add_utterance(text, reinforce=reinforce)
        if uid is None:
            return None
        ...
        #
        # Only reinforcing input teaches Bucket things. It still stores its own
        # replies so they remain searchable, but does not train on them.
        if not reinforce:
            return uid
        ...
        # and stripped of any context. The check is content-based because names
        # can arrive in any message, including a present-day question about one.
        ...
        self._learn_factoid(text)
        self._learn_item(text)
        return uid
    ...
    # ------------------------------------------------------------------
    def _learn_factoid(self, text: str) -> None:
        ...
        for sentence in SENTENCE_RE.findall(candidate):
            ...
            self._extract_factoid(cleaned)

    def _extract_factoid(self, candidate: str) -> None:
        ...
        self.db.add_factoid(subject, VERB_CANONICAL.get(verb, verb), obj)

    # ------------------------------------------------------------------
    def _learn_item(self, text: str) -> str | None:
        ...
        if event.action == INVENTORY_RECEIVE:
            return self.db.add_item(event.item, config.INVENTORY_SIZE)
