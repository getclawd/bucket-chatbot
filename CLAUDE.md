...
python run_all.py             # both chat surfaces + Mini App, one shared Bucket
...
python factoid_test.py && python identity_test.py && python webapp_test.py && \
...
An `aliases` table lets multiple names/handles resolve to one canonical
subject (`BucketDB.resolve()`), so facts taught under a Discord handle and a
Telegram display name merge. `display_name()` picks the shortest known alias
for display, since the canonical form is whichever name had more attached to
it (often a handle, not what people actually call each other). Any code that
prints a `factoids.subject` value for a human to read should go through
`display_name()`, not the raw column — see the strategies in `brain.py` for the
pattern.
...
`collision`, `attribution` — weld two things it knows together) and
...
collides across platforms. `bucket/lock.py` provides a cross-process advisory
...
`bucket/tunnel.py` wraps `cloudflared` to expose the Mini App
(`webapp/server.py`, a stdlib `ThreadingHTTPServer`) over a public HTTPS URL,
since Telegram Mini Apps refuse `localhost`. The tunnel URL is ephemeral by
design (a fresh one every launch) — don't pin `BUCKET_WEBAPP_URL` in `.env`
unless using a permanent named tunnel; `run_all.py` re-registers the Telegram
menu button with the fresh URL on every startup. The webapp's Telegram auth
(`webapp/server.py: verify_init_data`) validates the HMAC signature Telegram
attaches to Mini App requests against the bot token — this is the only auth
mechanism, there is no separate login.
