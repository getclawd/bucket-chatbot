...
The interesting case is a blocked name repeated in present-day content. Filtering
by text is the only thing that catches it, because the privacy-first database
does not retain speaker or chat provenance.
...
def main() -> int:
    ...
    # Present-day content mentioning a blocked name must not enter generation.
    for line in ["who is oldname_7", "oldname_7 is my friend", "pengu demands blood"]:
        bot.handle(line, chat="dc:1", is_private=True)
